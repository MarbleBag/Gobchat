...
namespace Gobchat.Module.Updater
{
    public sealed class GobVersion : ICloneable, IComparable, IComparable<GobVersion>, IEquatable<GobVersion>
    {
        ...
        public GobVersion(Version version) : this((uint)Math.Max(0, version.Major), (uint)Math.Max(0, version.Minor), (uint)Math.Max(0, version.Build))
        {
        }
        ...
        private static uint Parse(GroupCollection collection, string groupName)
        {
            ...
            return capture.Success ? UInt32.Parse(capture.Value, CultureInfo.InvariantCulture) : 0;
        }
